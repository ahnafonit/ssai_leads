...
function mapApolloPersonRecord(person) {
    const fromParts = [person.first_name, person.last_name].filter(Boolean).join(' ').trim();
    const ownerName =
...
function buildApolloPeopleRequestBody(filters) {
    const requestBody = {};
...
/**
 * People discovery: POST /mixed_people/api_search (Apollo’s current endpoint).
 * Legacy mixed_people/search is deprecated for API keys — not called here.
 */
async function searchApolloPeople(filters) {
    ...
    const requestBody = buildApolloPeopleRequestBody(filters);
    ...
    return raw.map(mapApolloPersonRecord);
}
